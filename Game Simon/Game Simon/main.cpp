...
int main()
{
	...
	if (registr)
	{
		...
		if (choice == 1)
		{
			...
			file << name << std::endl << password << std::endl << 0;
			file.close();
	...
	std::string fileName("D:\\PR\\Project\\Visual Studio (ready)\\Game Simon\\Data\\" + name + ".txt");
	std::string record;
	getline(read, record);
	maxScore = std::stoi(record);
	...
	guideText.setFillColor(Color(255, 255, 255, 180));
	...
	Text maxScoreText("", font, 60);
	maxScoreText.setFillColor(Color(255, 255, 255));
	maxScoreText.setPosition(coord.x - 550, coord.y - 100);
	...
	while (window.isOpen())
	{
				...
				if (event.key.code == Mouse::Left)
				{
					...
					if (play)
					{
						...
						case 1:
							...
							{
								if (maxScore > std::stoi(record))
								{
									remove(fileName.c_str());

									File.open(fileName.c_str());
									File << name << std::endl << password << std::endl << maxScore;
									File.close();
								}
		...
		std::ostringstream scoreString, timeString, maxScoreString;
		scoreString << score; timeString << allTimer; maxScoreString << maxScore;
		scoreText.setString("Score: " + scoreString.str());
		timeText.setString("Time: " + timeString.str());
		maxScoreText.setString("Record score: " + maxScoreString.str());
